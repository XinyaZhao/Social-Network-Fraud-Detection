...
def getGraph(costmap : Map[String, Double]) : List[String] = {
	var g = List[String]()
	for (c <- costmap){
		g = c._1::g
	}
...
def greedyDecreasing(degree : org.apache.spark.rdd.RDD[String]) : List[String] = {
	...
	val maxValue_index = valueMap.maxBy(_._2)._1
	println("maxvalue" + " " + valueMap.maxBy(_._2)._2 + " index:" + maxValue_index)
	//graph.reverse.foreach(println)
	val objectGraph = graph.reverse(maxValue_index)
	objectGraph
}
...
def outputResult(set : Set[String]) : Unit = {
	...
	val sorted_list = set.toList.sortWith(_.substring(1).toInt<_.substring(1).toInt)
	for( s <- sorted_list){
		if(s.contains("i")){
			ibf.write(s + "\n");
		}
